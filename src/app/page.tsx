...
export default function Home() {
  ...
  const handleSaveEntry = (data: TableData) => {
    ...
    setIsLoading(true);
    axiosClient
      .post("/ru/data/v3/testmethods/docs/userdocs/create", tableDataWithoutId)
      .then((response) => {
  ...
  const handleEditEntry = (data: TableData) => {
    ...
    setIsLoading(true);
    axiosClient
      .post(`/ru/data/v3/testmethods/docs/userdocs/set/${data.id}`, data)
      .then((response) => {
        if (response.data.error_code == 0) {
  ...
  return (
    <Container maxWidth="lg" className="mt-10">
      ...
      {isLoading ? <SkeletonTable /> : <DocumentTable data={data} onEdit={onEdit} onDelete={handleDeleteEntry} />}
      <Button
        className="mt-4 mx-2"
        onClick={() => {
          handleOpenModal("add");
